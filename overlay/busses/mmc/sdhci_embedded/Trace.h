...
// Per-object labeled tracing. Every log line is decorated with a short label
// (e.g. "sdhci_emb:eMMC0") so it is always obvious which device -- and that it
// is definitively the embedded driver, not upstream sdhci -- a line is about.
//
// The label is owned by each Controller/Card/Disk and threaded into these
// macros. Data-path TRACE is compile-gated to keep the syslog quiet during
// transfers; TRACE_ALWAYS and ERROR are always on.
