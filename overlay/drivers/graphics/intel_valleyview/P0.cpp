...
status_t
AllocateP0Area(const char* name, size_t size, bool cloneable, area_id& area,
	void*& address, phys_addr_t& physical)
{
...
AllocateP0Memory(ValleyViewDevice& device)
{
	...
	status_t status = AllocateP0Area("intel_valleyview P0 framebuffer",
		valleyview::kP0FramebufferBytes, true, device.framebufferArea,
		device.framebuffer, device.p0Physical);
	if (status != B_OK)
		return status;
	status = AllocateP0Area("intel_valleyview P0 private",
		valleyview::kP0PrivateBytes, false, device.p0PrivateArea,
		device.p0Private, device.p0PrivatePhysical);
	if (status != B_OK)
		return status;
	...
	for (uint32 page = 0; page < device.p0Layout.pageCount; page++) {
		...
		if (!valleyview::P0PagePhysical(device.p0Physical,
			...
			|| !valleyview::EncodeBytPte(physical, true, false, pte)) {
			return B_BAD_DATA;
		}
...
InitializeP0(ValleyViewDevice& device)
{
	...
	dprintf("intel_valleyview: P0 native scanout %ux%u stride=%u ggtt=%#"
		B_PRIx32 " pages=%" B_PRIu32 " bcs=%" B_PRId32 "\n",
...
ShutdownP0(ValleyViewDevice& device)
{
	...
	if (cursorStatus == B_OK && bcsStatus == B_OK && planeStatus == B_OK)
	...
	if (cursorStatus != B_OK || bcsStatus != B_OK || planeStatus != B_OK
		...
		device.p0MemoryQuarantined = true;
		device.gpuFaulted = true;
		dprintf("intel_valleyview: quarantining P0 allocation; cursor=%"
			B_PRId32 " bcs=%" B_PRId32 " plane=%" B_PRId32
			" ggtt=%" B_PRId32 "\n", cursorStatus, bcsStatus,
			planeStatus, pteStatus);
	}
	device.nativeStatus = status;
	free(device.p0SavedPtes);
	device.p0SavedPtes = NULL;
...
void
GetP0Status(const ValleyViewDevice& device, valleyview::P0Status& status)
{
