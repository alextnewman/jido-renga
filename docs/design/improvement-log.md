...
### 10.1 Revision determination fires *before* UserBuildConfig — preset Jam's environment
...
- **Fix:** `tools/jr-jam` composes and exports `HAIKU_REVISION` before it
  `exec`s Jam. Jam imports environment variables at process startup, before
  `Jamrules`, so the revision is available to both `DefaultBuildProfiles` and
  the repository walk. An explicit caller-provided `HAIKU_REVISION` still wins.
  `UserBuildConfig` remains exclusively the overlay graft.
- **Retired approach:** an intermediate implementation had `tools/weave` edit
  `<output>/build/BuildConfig`. It worked but duplicated the wrapper and
  violated the cleaner single-file graft contract. Revision delivery now has no
  persistent build-tree mutation beyond Jam's own revision cache, which
  `jr-jam` invalidates only when the requested value changes.
